...
def convertLine(shabadLine):
	shabad_unicode = []
	# if the tukh has "ir", we should swap the characters or else the output is incorrect
	for i in range(len(shabadLine)):
		index_to_convert = i
		converted_letter = convert(shabadLine[index_to_convert])
		shabad_unicode.append(converted_letter)
